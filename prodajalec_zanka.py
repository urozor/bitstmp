# v zanki preverja vrednosti in status narocil in avtomatsko kupuje in prodaja BCH
import urllib, urllib2, json 
import hmac
import hashlib
import time

prodajalec_lastprice="lastprice_BCH.txt"
delta=0.99 #koliko evrov razlike lovimo
...
while True:
	...
	url = "https://www.bitstamp.net/api/v2/open_orders/bcheur/"
	...
	if stevilo==0:   # ce ne obstaja odprtih narocil, nadaljuj
		...
		print(d)
		data = json.loads(d)
		bchjev=float(data['bch_available'])
		evrov=float(data['eur_available'])
		fee=0
		#fee=float(data['bcheur_fee'])/100+0.0001
		...
		if evrov>5: #kupi BCH, to je prioriteta		
			...
			if last<(zadnjaprodajna-delta): price=last
			if last>=(zadnjaprodajna-delta): price=zadnjaprodajna-delta
